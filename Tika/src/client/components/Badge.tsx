...
export const PriorityBadge = ({ priority }: PriorityBadgeProps) => {
  return (
    <span
      ...
      className={`rounded-card px-2 py-0.5 text-xs font-medium ${PRIORITY_CLASS[priority]}`}
    >
...
export const DueDateBadge = ({ dueDate, isOverdue = false }: DueDateBadgeProps) => {
  ...
  return (
    <span className={`rounded-card px-2 py-0.5 text-xs font-medium ${colorClass}`}>{dueDate}</span>
  );
};
